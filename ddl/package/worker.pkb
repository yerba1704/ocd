...
----------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  begin
    -- split_package always delete SCHEMA_PACKAGE (cascade), then parse package content and rebuild table
    -- because of that, sync_to_content_revision (needed for further processsing in apex gui) is always a simple insert
    -- any possible gui modifications between two compilations are temporarily stored in a global temporary table
...
--      where jsn_base.schema_name=c_schema_name and jsn_base.package_name=c_package_name
    ...
    lv_out clob;
  begin
    execute immediate c_stmt into lv_out using c_schema_name, c_package_name, c_schema_name, c_package_name, c_schema_name, c_package_name;
    
    return lv_out;
  exception
    when no_data_found then 
      return to_clob('{"name":"'||c_package_name||'","status":"NO_DATA_FOUND"}');
    when others then
      raise;
  end information;
----------------------------------------------------------------------------------------------------------------------------------------------------------------
